...
|---|---|---|
| `dataprep` | AIMD frame selection, DFT extraction, `.data` (RuNNer/n2p2) I/O, CP2K coulomb calc, charge decoupling | `from dataprep import ...` |
...
```
 ...
 3  charge decouple     run_coulomb() + decouple    [example/3_charge_decouple/]
      ...
      ▼
    train the MLFF (n2p2, external)  ── short-range (non-ES) target; ES re-added
      │                                 by mean-field coupling at run time
      ▼
 4  QM/ML run           run(Config(...))            [example/4_qmml_run/]
```
...
driver needs no compiled cube tools. `example/4_qmml_run/run_qmml.py` is just configuration:
...
Run with `python example/4_qmml_run/run_qmml.py` after filling in the values marked `REPLACE`.
